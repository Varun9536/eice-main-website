import React from "react";
function Copyright()
{
    return (
        <div style={{ background : "rgba(1, 32, 96)"}}>
            {/* <div className="sm:max-w-7xl w-screen mx-auto px-4">
                <div className="pt-1 shadow-sm shadow-blackk/30"></div>
                <div className="grid grid-cols justify-end max-w-screen-2xl 
                                2xl:p-2 2xl:text-base 
                                p-1 text-[10px]">
                    <div style={{textAlign : "center"}} className="">© 2024 EICE Technology</div>
                    
                </div>
            </div> */}
          
              <div    style={{textAlign : "center" , color : "white", fontSize : "12px"}} >Copyright ©&nbsp; EICE Technologies 2024. All Rights Reserved.</div>
              {/* <p>Copyright ©&nbsp; Technologies 2023. All Rights Reserved.</p> */}
        </div>
    )
}

export default Copyright;